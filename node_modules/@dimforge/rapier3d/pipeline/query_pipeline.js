...
var QueryPipeline = /** @class */ (function () {
    ...
    QueryPipeline.prototype.castRay = function (colliders, ray, maxToi, solid, groups) {
        var rawOrig = VectorOps.intoRaw(ray.origin);
        var rawDir = VectorOps.intoRaw(ray.dir);
        var result = RayColliderToi.fromRaw(this.raw.castRay(colliders.raw, rawOrig, rawDir, maxToi, solid, groups));
        rawOrig.free();
        rawDir.free();
        return result;
    };
    ...
    QueryPipeline.prototype.castRayAndGetNormal = function (colliders, ray, maxToi, solid, groups) {
        var rawOrig = VectorOps.intoRaw(ray.origin);
        var rawDir = VectorOps.intoRaw(ray.dir);
        var result = RayColliderIntersection.fromRaw(this.raw.castRayAndGetNormal(colliders.raw, rawOrig, rawDir, maxToi, solid, groups));
        rawOrig.free();
        rawDir.free();
        return result;
    };
    ...
    QueryPipeline.prototype.intersectionWithShape = function (colliders, shapePos, shapeRot, shape, groups) {
        var rawPos = VectorOps.intoRaw(shapePos);
        var rawRot = RotationOps.intoRaw(shapeRot);
    ...
    QueryPipeline.prototype.projectPoint = function (colliders, point, solid, groups) {
        ...
        var result = PointColliderProjection.fromRaw(this.raw.projectPoint(colliders.raw, rawPoint, solid, groups));
        rawPoint.free();
        return result;
    };
    /**
    ...
    QueryPipeline.prototype.castShape = function (colliders, shapePos, shapeRot, shapeVel, shape, maxToi, groups) {
        var rawPos = VectorOps.intoRaw(shapePos);
        var rawRot = RotationOps.intoRaw(shapeRot);
        var rawVel = VectorOps.intoRaw(shapeVel);
        var rawShape = shape.intoRaw();
        var result = ShapeColliderTOI.fromRaw(this.raw.castShape(colliders.raw, rawPos, rawRot, rawVel, rawShape, maxToi, groups));
        rawPos.free();
        rawRot.free();
        rawVel.free();
        rawShape.free();
